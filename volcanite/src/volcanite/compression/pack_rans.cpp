...
void SymbolStats::normalize_freqs(uint32_t target_total) {
    ...
    calc_cum_freqs();
    uint32_t cur_total = cum_freqs[RANS_ALPHABET_SIZE];
...
uint32_t RANS::packRANS(std::vector<uint32_t> &in_packed, uint32_t start4bit, uint32_t end4bit) const {
    ...
    for (int i = end4bit - 1; i >= start4bit; i--) { // NB: working in reverse!
        ...
        uint32_t s = (in_packed[i / 8] >> shift) & 0xFu;
    ...
    int new_size_in_bytes = static_cast<int>(out_buf + out_max_size - rans_begin);
    ...
    uint32_t start32bit = start4bit / 8u;
    uint32_t end32bit = start32bit + (new_size_in_bytes + sizeof(uint32_t) - 1) / sizeof(uint32_t); // round our byte_count up so we use exactly a number of uint32_ts
    uint32_t out = 0u;
    uint32_t i = 0;
...
int RANS::unpackRANS(uint8_t *rans_begin, uint8_t *out, size_t number_of_output_elements) const {
    // rANS decode --------------------------------------------------------------
