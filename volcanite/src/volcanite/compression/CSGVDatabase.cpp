        ...
        try{
            ...
            if(attribute_database.empty()) {
                ...
                if (attribute_database.ends_with(".csv")) {
                    ...
                    db.exec("ATTACH DATABASE 'file::memory:' AS attr_db");
                    ...
                    create_table_sql_csv << "CREATE TABLE attr_db." << IN_MEMORY_TABLE << "(" ;

                    insert_sql_csv << "INSERT INTO attr_db." << IN_MEMORY_TABLE << "(";
                    {
                        // read attribute names
                        bool first_column = true;
                        int look_at_first_count_entries = std::min(100, static_cast<int>(csv_database.size()));
                        for (int attribute_idx = 0; attribute_idx < attribute_names.size(); attribute_idx++) {
                            ...
                            // check what data type the column has by looking at the first count values
                            bool is_float = false;
                            for (int i = 0; i < look_at_first_count_entries; i++) {
                                float val_temp = csv_database[i][attribute_idx];
                                if (val_temp != std::floor(val_temp)) {
                                    create_table_sql_csv << attribute_names[attribute_idx] << " REAL";
                                    is_float = true;
                                    break;
                                }
                            }
                            ...
                            if (!is_float)
                                create_table_sql_csv << attribute_names[attribute_idx] << " INTEGER";
                        ...
                        db.exec("DROP TABLE IF EXISTS attr_db." + IN_MEMORY_TABLE);
