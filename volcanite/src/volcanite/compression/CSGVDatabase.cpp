...
namespace volcanite {
    ...
    bool CSGVDatabase::databaseExportAndOpen(const std::string& sqlite_path, const std::vector<uint32_t>& index_to_label,
        ...
        try{
            ...
            {
                db.exec("CREATE TABLE " + CSGV_ATTRIBUTE_TABLE + "_tmp (" + ID_COLUMN + " UNSIGNED INT PRIMARY KEY, "
                        + csgv_label_name + " UNSIGNED INT UNIQUE)");
                ...
                if (attribute_database.ends_with(".csv")) {
                    ...
                    insert_sql_csv << "INSERT INTO attr_db." << attribute_table << "(";
                    {
                        ...
                        for (int attribute_idx = 0; attribute_idx < attribute_names.size(); attribute_idx++) {
                            if (attribute_names[attribute_idx] == label_column) label_column_index = attribute_idx;
                            if (!first_column) create_table_sql_csv << ", ";
                            ...
                            for (int i = 0; i < csv_database.size(); i++) {
                                if (is_float)
                                    continue;
                                float val_temp = csv_database[i][attribute_idx];
                            ...
                            if (!first_column) insert_sql_csv << ", ";
