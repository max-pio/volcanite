...
public:
    ...
    void writeNrrd(std::string path, bool separatePayloadFile = true);
    ...
    static std::shared_ptr<Volume<ElementType, HolderType>> load_simple_cellsinsilico(std::string path, bool allowCast = false);

    void write_simple_cellsinsilico(std::string path);
