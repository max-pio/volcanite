...
shade_pt = [([], "_pt", 0.5)]
# other default args
def_volc = [(["--verbose", "--headless", "--shader-def", "CSGV_UNDECODED_RANDOM_LABEL"], "", 1000)]
...
def volcanite(arg_args, fallback_log=None, eval_name=None):
    ...
    args = ["./volcanite", "--eval-logfiles", str(log_path)]
    if eval_name:
...
if __name__ == "__main__":
    ...
    shade_tex = ["local shading", "shadow rays", "ambient occlusion", "path tracing"]
    enc_tex = ["\\gls{csgv} Nibble", "\\gls{csgv} rANS", "\\gls{csgvr}", "\\gls{csgvr}+sb"]
    cache_mode_tex = ["no cache ", "voxel cache ", "voxel cache (es)", "brick cache ", "brick cache (sm) "]

    for enc_mode_i, enc_mode in enumerate([e_nibble, e_rans, e_wmh_nosb, e_wmh]):
        log_manual("\\midrule\n")

        if enc_mode == e_rans or enc_mode == e_nibble:
        ...
        for cache_mode_i, cache_mode in enumerate([cache_no, cache_voxel, cache_voxel_es, cache_brick, cache_brick_sm]):
            ...
            if (enc_mode == e_rans or enc_mode == e_nibble) and cache_mode != cache_brick:
                continue
