...
if __name__ == "__main__":
    ...
    evaluation = VolcaniteEvaluation(eval_out_directory=f"./results/{evaluation_name}/", existing_policy=ExistingPolicy.DELETE,
                                        ...
                                        log_files=[VolcaniteLogFileCfg(f"{evaluation_name}.csv",
                                                              fmts=["{name},{frame_min_ms},{frame_avg_ms},{frame_max_ms},{frame_sdv_ms},"
                                                                     + ",".join("{:02}".format(i) for i in range(16))],
                                                              headers=["Shading Mode,frame min [ms],frame avg [ms],frame max [ms],stdv,"
    ...
    for arg_data in VolcaniteArg.args_csgv_datasets.values():
        ...
        for arg_shading in VolcaniteArg.args_shading.values():
            ...
            image_export = VolcaniteArg.arg_image_export([arg_data, arg_shading])

            vargs = [arg_data, render_config, arg_shading, image_export] + data_specific_args(arg_data.identifier)
