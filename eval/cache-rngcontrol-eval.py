...
if __name__ == "__main__":
    ...
    with VolcaniteEvaluation(eval_out_directory=f"./results/{evaluation_name}/", existing_policy=ExistingPolicy.DELETE,
                                        ...
                                        log_files=[VolcaniteLogFileCfg(f"{evaluation_name}.csv",
                                                              ...
                                                              headers=["Data Set,Shading Mode,RNG Control,Cache Size [MB],Used Size [MB],Used Size [Pcnt],frame min [ms],frame avg [ms],frame max [ms],stdv,frame med [ms],SPP min,SPP max"])],
        ...
        for arg_data in VolcaniteArg.args_csgv_datasets.values():
            ...
            for arg_shading in VolcaniteArg.args_shading.values():
                ...
                for arg_mode in [VolcaniteArg(["--shader-def", "UNBIASED_RNG"], "", 172),
                                 VolcaniteArg([], "rngctrl", 172)]:
                    ...
                    vargs = [arg_data, arg_vcfg, arg_shading, arg_mode, \
                            ...
                            + data_specific_rendering_args(arg_data.identifier, cache_size=False) # cache_palette=False ?
                    ...
                    evaluation.get_log().log_manual(arg_data.identifier + "," + arg_shading.identifier + "," + ("yes" if arg_mode.identifier == "rngctrl" else "no") + ",", end="")
