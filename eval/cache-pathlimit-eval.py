...
if __name__ == "__main__":
    ...
    with VolcaniteEvaluation(eval_out_directory=f"./results/{evaluation_name}/", existing_policy=ExistingPolicy.DELETE,
                                        ...
                                        log_files=[VolcaniteLogFileCfg(f"{evaluation_name}.csv",
                                                              ...
                                                              headers=["Data Set,Shading Mode,Request Path Length,Cache Size [MB],Used Size [MB],Used Size [Pcnt],frame min [ms],frame avg [ms],frame max [ms],stdv,frame med [ms],SPP min,SPP max"])],
        ...
        for arg_data in VolcaniteArg.args_csgv_datasets.values():
            ...
            for arg_shading in VolcaniteArg.args_shading.values():
                ...
                if arg_shading.identifier == "local" or arg_shading.identifier == "ao":
                    continue

                for path_length_limit in [0, 1, 32]:
                    arg_mode = VolcaniteArg(["--config", f"[Display] Decompression_Path_Length: {path_length_limit}"], "reqpathl" + str(path_length_limit), 173)
                    ...
                    vargs = [arg_data, arg_vcfg, arg_shading, arg_mode, \
                            ...
                            + data_specific_rendering_args(arg_data.identifier)
                    ...
                    evaluation.get_log().log_manual(arg_data.identifier + "," + arg_shading.identifier + "," + str(path_length_limit) + ",", end="")
