...
if __name__ == "__main__":
    ...
    with VolcaniteEvaluation(eval_out_directory=f"./results/{evaluation_name}/", existing_policy=ExistingPolicy.DELETE,
                                        ...
                                        log_files=[VolcaniteLogFileCfg(f"{evaluation_name}.csv",
                                                              fmts=["{mem_cache_mb},{mem_cache_used_mb},{mem_cache_fillrate_pcnt},{frame_min_ms},{frame_avg_ms},{frame_max_ms},{frame_sdv_ms},{frame_med_ms},{min_spp},{max_spp}"],
                                                              headers=["Data Set,Shading Mode,Request Limitation,Cache Size [MB],Used Size [MB],Used Size [Pcnt],frame min [ms],frame avg [ms],frame max [ms],stdv,frame med [ms],SPP min,SPP max"])],
        ...
        for arg_data in VolcaniteArg.args_csgv_datasets.values():
            ...
            for arg_shading in VolcaniteArg.args_shading.values():
                ...
                for arg_mode in [VolcaniteArg(["--config", "[Development] Auto_Cache_Request_Limitation: 0"], "", 171),
                                 VolcaniteArg(["--config", "[Development] Auto_Cache_Request_Limitation: 1"], "reqlim", 171)]:
                    ...
                    vargs = [arg_data, arg_vcfg, arg_shading, arg_mode, \
                             arg_timing_export, arg_image_eval, arg_image_export, VolcaniteArg.arg_cache_size(512)] \
                            + data_specific_rendering_args(arg_data.identifier, cache_size=False)
                    ...
                    evaluation.get_log().log_manual(arg_data.identifier + "," + arg_shading.identifier + "," + ("yes" if arg_mode.identifier == "reqlim" else "no") + ",", end="")
