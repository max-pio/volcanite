...
// original code: https://github.com/boksajak/brdf
// article: https://boksajak.github.io/blog/BRDF
// adaptions by Max Piochowiak: GLSL compatibility
...
//inline vec2 mul(vec2x2 a, vec2 b) { return b * a; }
//inline vec3 mul(vec3x3 a, vec3 b) { return b * a; }
//
//#else
...
FUNCTION float luminance(vec3 rgb)
{
	return dot(rgb, vec3(0.2126f, 0.7152f, 0.0722f));
}

FUNCTION vec3 baseColorToSpecularF0(const vec3 baseColor, const float metalness, const float reflectance) {
...
#endif
	return lerp(vec3(minDielectricsF0, minDielectricsF0, minDielectricsF0), baseColor, metalness);
}

FUNCTION vec3 baseColorToDiffuseReflectance(vec3 baseColor, float metalness)
{
...
FUNCTION vec3 evalVoid(const BrdfData data) {
	return vec3(0.0f, 0.0f, 0.0f);
}

FUNCTION void evalIndirectVoid(const BrdfData data, vec2 u, OUT_PARAMETER(vec3) rayDirection, OUT_PARAMETER(vec3) weight) {
	rayDirection = vec3(0.0f, 0.0f, 1.0f);
	weight = vec3(0.0f, 0.0f, 0.0f);
}

FUNCTION vec3 sampleSpecularVoid(vec3 Vlocal, float alpha, float alphaSquared, vec3 specularF0, vec2 u, OUT_PARAMETER(vec3) weight) {
	weight = vec3(0.0f, 0.0f, 0.0f);
	return vec3(0.0f, 0.0f, 0.0f);
}

FUNCTION vec3 sampleSpecularHalfVectorVoid(vec3 Vlocal, vec2 alpha2D, vec2 u) {
	return vec3(0.0f, 0.0f, 0.0f);
}
...
FUNCTION vec4 getRotationToZAxis(vec3 dir_in) {

	// Handle special case when input is exact or near opposite of (0, 0, 1)
	if (dir_in.z < -0.99999f) return vec4(1.0f, 0.0f, 0.0f, 0.0f);

	return normalize(vec4(dir_in.y, -dir_in.x, 0.0f, 1.0f + dir_in.z));
}

// Calculates rotation quaternion from vector (0, 0, 1) to the input vector
// Input vector must be normalized!
FUNCTION vec4 getRotationFromZAxis(vec3 dir_in) {
...
FUNCTION vec4 invertRotation(vec4 q)
{
...
FUNCTION vec3 rotatePoint(vec4 q, vec3 v) {
...
FUNCTION vec3 sampleHemisphere(vec2 u, OUT_PARAMETER(float) pdf) {
...
FUNCTION vec3 sampleHemisphere(vec2 u) {
	float pdf;
...
FUNCTION vec3 evalFresnelSchlick(vec3 f0, float f90, float NdotS)
{
...
FUNCTION vec3 evalFresnelSchlickSphericalGaussian(vec3 f0, float f90, float NdotV)
{
...
FUNCTION vec3 evalFresnelHoffman(vec3 f0, float f82, float f90, float NdotS)
{
...
FUNCTION vec3 evalFresnel(vec3 f0, float f90, float NdotS)
{
...
FUNCTION float shadowedF90(vec3 F0) {
...
FUNCTION vec3 evalLambertian(const BrdfData data) {
...
FUNCTION vec3 evalPhong(const BrdfData data) {
...
// Source: "Sampling Transformations Zoo" in Ray Tracing Gems by Shirley et al.
FUNCTION vec3 samplePhong(vec3 Vlocal, float shininess, vec2 u, OUT_PARAMETER(float) pdf) {
...
FUNCTION vec3 samplePhong(vec3 Vlocal, vec2 alpha2D, vec2 u) {
...
FUNCTION vec3 sampleSpecularPhong(vec3 Vlocal, float alpha, float alphaSquared, vec3 specularF0, vec2 u, OUT_PARAMETER(vec3) weight) {
...
FUNCTION vec3 evalOrenNayar(const BrdfData data) {
...
FUNCTION vec3 evalDisneyDiffuse(const BrdfData data) {
...
FUNCTION vec3 evalFrostbiteDisneyDiffuse(const BrdfData data) {
...
// PDF is 'G1(NdotV) * D'
FUNCTION vec3 sampleGGXVNDF(vec3 Ve, vec2 alpha2D, vec2 u) {
...
// Source: "Microfacet Models for Refraction through Rough Surfaces" by Walter et al.
// PDF is 'D * NdotH'
FUNCTION vec3 sampleBeckmannWalter(vec3 Vlocal, vec2 alpha2D, vec2 u) {
...
// Source: "Microfacet Models for Refraction through Rough Surfaces" by Walter et al.
// PDF is 'D * NdotH'
FUNCTION vec3 sampleGGXWalter(vec3 Vlocal, vec2 alpha2D, vec2 u) {
	float alpha = dot(alpha2D, vec2(0.5f, 0.5f));
...
FUNCTION vec3 sampleSpecularMicrofacet(vec3 Vlocal, float alpha, float alphaSquared, vec3 specularF0, vec2 u, OUT_PARAMETER(vec3) weight) {
...
FUNCTION vec3 evalMicrofacet(const BrdfData data) {
...
FUNCTION BrdfData prepareBRDFData(vec3 N, vec3 L, vec3 V, MaterialProperties material) {
	BrdfData data;
...
FUNCTION vec3 evalCombinedBRDF(vec3 N, vec3 L, vec3 V, MaterialProperties material) {
...
FUNCTION bool evalIndirectCombinedBRDF(vec2 u, vec3 shadingNormal, vec3 geometryNormal, vec3 V, MaterialProperties material, const int brdfType, OUT_PARAMETER(vec3) rayDirection, OUT_PARAMETER(vec3) sampleWeight) {
	...
	if (brdfType == DIFFUSE_TYPE) {
		...
		if (VdotH > 0.00001f)
		{
			sampleWeight *= (vec3(1.0f, 1.0f, 1.0f) - evalFresnel(data.specularF0, shadowedF90(data.specularF0), min(1.0f, VdotH)));
		} else {
			return false;
		}
#else
...
FUNCTION vec3 specularGGXReflectanceApprox(vec3 specularF0, float alpha, float NdotV)
{
#if USE_HEIGHT_CORRELATED_G2
	const vec2x2 A = vec2x2(
		0.995367f, -1.38839f,
		-0.24751f, 1.97442f
	);

	const vec3x3 B = vec3x3(
		1.0f, 2.68132f, 52.366f,
		16.0932f, -3.98452f, 59.3013f,
		-5.18731f, 255.259f, 2544.07f
	);

	const vec2x2 C = vec2x2(
		-0.0564526f, 3.82901f,
		16.91f, -11.0303f
	);

	const vec3x3 D = vec3x3(
	...
	const float E = dot(mul(A, vec2(1.0f, NdotV)), vec2(1.0f, alpha));
	const float F = dot(mul(B, vec3(1.0f, NdotV, NdotV3)), vec3(1.0f, alpha, alpha3));

	const float G = dot(mul(C, vec2(1.0f, NdotV)), vec2(1.0f, alpha));
	const float H = dot(mul(D, vec3(1.0f, NdotV2, NdotV3)), vec3(1.0f, alpha, alpha3));

	// Turn the bias off for near-zero specular
	const float biasModifier = saturate(dot(specularF0, vec3(0.333333f, 0.333333f, 0.333333f)) * 50.0f);

	const float bias = max(0.0f, (E / F)) * biasModifier;
	const float scale = max(0.0f, (G / H));

	return vec3(bias, bias, bias) + vec3(scale, scale, scale) * specularF0;
}
