...
(CC0 license). In principle, it slightly adapts the existing code for rANS encoding to operate on uint32 input
arrays instead of byte pointers. Most parts of the file - including original comments - remain unchanged from
...
// ----------- rANS decode GPU suitable ------------------------------------------------
static inline void RansDecInit(uint32_t& rans_state, uint32_t& byte_index, const uint32_t* array) {
    rans_state = array[byte_index/4];
    byte_index += 4;
}
...
static inline void RansDecAdvance(RansState& r, uint32_t& byte_index,  const uint32_t* array, uint32_t start, uint32_t freq, uint32_t scale_bits)
{
    ...
    // renormalize
    if (x < RANS_BYTE_L) {
        uint32_t idx = byte_index;
        do {
            // read the next byte from our uint32 array
            int shift = (idx % 4) * 8;
            uint32_t byte = (array[idx / 4] >> shift) & 0xFFu;
            x = (x << 8) | byte;
            idx++;
        } while (x < RANS_BYTE_L);
        byte_index = idx;
...
static inline void RansDecAdvanceSymbol(RansState& r, uint32_t& byte_index, const uint32_t* array,  uint32_t sym_start, uint32_t sym_freq, uint32_t scale_bits)
{
    RansDecAdvance(r, byte_index, array, sym_start, sym_freq, scale_bits);
}


// end ----------- rANS decode GPU suitable --------------------------------------------
